...
uint8_t ds18b20_Reset(void) {
    ...
    D0(); delay_us(500); // min 480 us
    D1(); delay_us(70); status = GPIO_ReadInputDataBit(DS18B20_dataPort, DS18B20_dataPin); 
    D1(); delay_us(430);
    return status;
}
...
uint8_t ds18b20_ReadByte(void) {
    static uint8_t data;
    data = 0;
    for (uint8_t i = 0; i <= 7; i++) data += ds18b20_ReadBit() << i;
    return data;
}
...
void ds18b20_WriteByte(uint8_t dt) {
    for (uint8_t i = 0; i < 8; i++) ds18b20_WriteBit(dt >> i & 1); 
}

/// @brief Send to 1Wire command to convert temperature
void ds18b20_StartMeasure() {
    __disable_irq();
    ds18b20_Reset(); ds18b20_WriteByte(0xCC); // SKIP_ROM
...
void ds18b20_ReadStratcpad(uint8_t n) {
    static uint8_t i;
    __disable_irq();
    ds18b20_Reset(); ds18b20_WriteByte(0xCC);
    ds18b20_WriteByte(0xBE); //READ SCRATCHPAD
    for(i = 0; i < n; i++) Scratchpad[i] = ds18b20_ReadByte();
    __enable_irq();
}
...
void ds18b20_ReadStratcpadAddr(uint8_t n, uint8_t addr[]) {
    static uint8_t i;
    __disable_irq();
    ds18b20_Reset(); ds18b20_WriteByte(0x55);for(uint8_t a = 0; a < 8; a++) ds18b20_WriteByte(addr[a]);
    ds18b20_WriteByte(0xBE); //READ SCRATCHPAD
    for(i = 0; i < n; i++) Scratchpad[i] = ds18b20_ReadByte();
    __enable_irq();
}
...
float bytes_to_temp(uint8_t a, uint8_t b){
    ...
    if( ((1 << 0)  &  (b & 0x0F)) ) temp += 0.0625;
    if( ((1 << 1)  &  (b & 0x0F)) ) temp += 0.125;
    if( ((1 << 2)  &  (b & 0x0F)) ) temp += 0.250;
    if( ((1 << 3)  &  (b & 0x0F)) ) temp += 0.500;
    return temp;
}
...
int8_t get_integer(float input){
    if(input < 0) return (uint8_t)(input * -1); 
...
float ds18b20_get_temperature_by_index(uint8_t id){
    float temp = -100.0;
    if (id > ds18b20_total_sensors) return temp;
...
void ds18b20_add_data_to_str(char *body){
    ...
    for(uint8_t i = 0; i < ds18b20_total_sensors; i++){
			...
			strcat(body, temp);
			if((i+1) == ds18b20_total_sensors) body[strlen(body) - 1] = '\0';
        }
    strcat(body, "}");
}
...
void ds18b20_manual_add_id(uint8_t id[8]){
    if(ds18b20_total_sensors < DS18B20_MAX_DEVICES){
        for(uint8_t i = 0; i < 8; i++) ds18b20_ids[ds18b20_total_sensors][i] = id[i];
        ds18b20_total_sensors++;
    }
}