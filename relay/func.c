...
void relay_int(){
	...
	if(relay_state[current_relay] != relay_target[current_relay]){
		if(relay_target[current_relay] == 0) {GPIO_ResetBits(Relay_ch1_Port, Relay_ch1_Pin);LEDOFF(current_relay+1);}
		else {GPIO_SetBits(Relay_ch1_Port, Relay_ch1_Pin);LEDON(current_relay+1);}
	...
	current_relay = 1;
	if(relay_state[current_relay] != relay_target[current_relay]){
		if(relay_target[current_relay] == 0) {GPIO_ResetBits(Relay_ch2_Port, Relay_ch2_Pin);LEDOFF(current_relay+1);}
		else {GPIO_SetBits(Relay_ch2_Port, Relay_ch2_Pin);LEDON(current_relay+1);}
		relay_state[current_relay] = relay_target[current_relay];
	}
	#endif
	...
	current_relay = 2;
	if(relay_state[current_relay] != relay_target[current_relay]){
		if(relay_target[current_relay] == 0) {GPIO_ResetBits(Relay_ch3_Port, Relay_ch3_Pin);LEDOFF(current_relay+1);}
		else {GPIO_SetBits(Relay_ch3_Port, Relay_ch3_Pin);LEDON(current_relay+1);}
		relay_state[current_relay] = relay_target[current_relay];
	}
	#endif
	...
	current_relay = 3;
	if(relay_state[current_relay] != relay_target[current_relay]){
		if(relay_target[current_relay] == 0) {GPIO_ResetBits(Relay_ch4_Port, Relay_ch4_Pin);LEDOFF(current_relay+1);}
		else {GPIO_SetBits(Relay_ch4_Port, Relay_ch4_Pin);LEDON(current_relay+1);}
		relay_state[current_relay] = relay_target[current_relay];
	}
	#endif
	...
	current_relay = 4;
	if(relay_state[current_relay] != relay_target[current_relay]){
		if(relay_target[current_relay] == 0) {GPIO_ResetBits(Relay_ch5_Port, Relay_ch5_Pin);LEDOFF(current_relay+1);}
		else {GPIO_SetBits(Relay_ch5_Port, Relay_ch5_Pin);LEDON(current_relay+1);}
		relay_state[current_relay] = relay_target[current_relay];
	}
	#endif
	...
	current_relay = 5;
	if(relay_state[current_relay] != relay_target[current_relay]){
		if(relay_target[current_relay] == 0) {GPIO_ResetBits(Relay_ch6_Port, Relay_ch6_Pin);LEDOFF(current_relay+1);}
		else {GPIO_SetBits(Relay_ch6_Port, Relay_ch6_Pin);LEDON(current_relay+1);}
		relay_state[current_relay] = relay_target[current_relay];
	}
	#endif
	...
	current_relay = 6;
	if(relay_state[current_relay] != relay_target[current_relay]){
		if(relay_target[current_relay] == 0) {GPIO_ResetBits(Relay_ch7_Port, Relay_ch7_Pin);LEDOFF(current_relay+1);}
		else {GPIO_SetBits(Relay_ch7_Port, Relay_ch7_Pin);LEDON(current_relay+1);}
		relay_state[current_relay] = relay_target[current_relay];
	}
	#endif
	...
	current_relay = 7;
	if(relay_state[current_relay] != relay_target[current_relay]){
		if(relay_target[current_relay] == 0) {GPIO_ResetBits(Relay_ch8_Port, Relay_ch8_Pin);LEDOFF(current_relay+1);}
		else {GPIO_SetBits(Relay_ch8_Port, Relay_ch8_Pin);LEDON(current_relay+1);}
		relay_state[current_relay] = relay_target[current_relay];
	}
	#endif
...
void RELAY_ON(uint8_t relay){
	if (relay > 0 && relay <= RELAY_NUM_OF_CHANNELS) relay_target[relay - 1] = 1;
}

/// @brief Turn relay OFF
/// @param relay number of relay (1...RELAY_NUM_OF_CHANNELS)
void RELAY_OFF(uint8_t relay){
	if (relay > 0 && relay <= RELAY_NUM_OF_CHANNELS) relay_target[relay - 1] = 0;
}
...
void RELAY_TOGGLE(uint8_t relay){
	if (relay > 0 && relay <= RELAY_NUM_OF_CHANNELS){
		if(relay_state[relay - 1]) relay_target[relay - 1] = 0;
		else relay_target[relay - 1] = 1;
	}
}
...
uint8_t relay_get_state(uint8_t relay){
	if (relay < 1 || relay > RELAY_NUM_OF_CHANNELS) return 0;
	if (relay_state[relay - 1] == 1) return 1;
	else return 0;
}
...
void relay_add_data_to_str(char *body){
    ...
    for(uint8_t i = 0; i < RELAY_NUM_OF_CHANNELS; i++){
			...
			strcat(body, temp);
			if((i+1) == RELAY_NUM_OF_CHANNELS) body[strlen(body) - 1] = '\0';
        }
	strcat(body, "]");
