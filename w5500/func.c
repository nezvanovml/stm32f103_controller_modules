...
int w5500_ini(){
	...
	w5500_initialized = 0;
	chip_reset();
	net_info.mac[0] = 0xAA;net_info.mac[1] = 0xBB;net_info.mac[2] = 0xCC;net_info.mac[3] = 0xDD;net_info.mac[4] = 0xEE;net_info.mac[5] = device_index;	
	#ifdef W5500_NETWORK
	net_info.ip[0] = network[0];net_info.ip[1] = network[1];net_info.ip[2] = network[2];net_info.ip[3] = device_index;
	net_info.sn[0] = netmask[0];net_info.sn[1] = netmask[1];net_info.sn[2] = netmask[2];net_info.sn[3] = netmask[3];
	net_info.gw[0] = network[0];net_info.gw[1] = network[1];net_info.gw[2] = network[2];net_info.gw[3] = 1;
...
int8_t parse_http_request(struct HttpRequest * http_request, char* buf) {
	...
	if(path_position < 0){ // Path not found
		return -2;
	}
	int16_t args_position = str_get_pos(buf, "?");
	int16_t proto_version_position = str_get_pos(buf, "HTTP/"); // HTTP/1.1
	if(args_position < 0 || args_position > proto_version_position){ // No need to parse args
		for(uint16_t i = 0; i < proto_version_position-path_position; i++){
			if(i+1 == MAX_PATH_SIZE || i == proto_version_position-path_position-1){http_request->path[i] = '\0';break;}
			else http_request->path[i] = buf[path_position+i];
		} 
		http_request->args[0] = '\0';
	} else { // Parsing args
		for(uint16_t i = 0; i < args_position-path_position+1; i++){
			if(i+1 == MAX_PATH_SIZE || i == args_position-path_position){http_request->path[i] = '\0';break;}
			else http_request->path[i] = buf[path_position+i];
		} 
		for(uint16_t i = 0; i < proto_version_position-args_position-1; i++){
			if(i+1 == MAX_ARGS_SIZE || i == proto_version_position-args_position-2){http_request->args[i] = '\0';break;} 
			else http_request->args[i] = buf[args_position+i+1];
		} 
	}

	// Parse Content-Length
	int16_t content_length_position = str_get_pos(buf,"Content-Length:");
	if(content_length_position < 0){ // Content-Length not found
		return 1;
	}
	...
	if(body_position > 0){
		...
		if (body_position + content_length >= DEFAULT_BUFFER_SIZE) content_length = DEFAULT_BUFFER_SIZE-body_position; // Limit body length
   ...
   switch(getSn_SR(sn))
   {
      case SOCK_ESTABLISHED :
		 ...
		 if((size = getSn_RX_RSR(sn)) > 0 && http_request->request[0] == '\0') // Don't need to check SOCKERR_BUSY because it doesn't not occur.
         {
			...
			if(ret <= 0) return HTTP_ERROR;      // check SOCKERR_BUSY & SOCKERR_XXX. For showing the occurrence of SOCKERR_BUSY.
